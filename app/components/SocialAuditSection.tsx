...
const AUDIT_STEPS = [
  { id: "fetch",   label: "Pulling recent Instagram posts" },
  { id: "analyze", label: "Analyzing hooks, formats, and pillars" },
  { id: "score",   label: "Scoring content health" },
  { id: "build",   label: "Building branded report" },
];
...
export default function SocialAuditSection({ businessName }: Props) {
  ...
  const runAudit = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/social-audit", {
        ...
        body: JSON.stringify({ handle: cleaned, business_name: businessName }),
      });
  ...
  return (
    <div className="bg-f10-tint rounded-f10 border border-f10-border p-6 mb-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <span className="font-heading text-xs tracking-widest uppercase text-f10-primary">Social</span>
          <h3 className="font-heading text-xl font-semibold text-f10-text">Content Strategy Report</h3>
        </div>
      ...
      {status === "idle" && (
        <div>
          <p className="font-body text-sm text-gray-400 mb-4 leading-relaxed">
            Pull this business&apos;s recent Instagram posts and analyze top performers, format performance, content pillars, and the hooks that win vs. flop. Generates a branded report you can send with the proposal.
          </p>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex items-center bg-f10-bg border border-f10-border rounded-lg px-3 flex-1">
              ...
              <input
                ...
                className="bg-transparent font-body text-sm text-f10-text placeholder-gray-600 px-2 py-2.5 flex-1 outline-none"
              />
            </div>
            <button
              onClick={runAudit}
              className="font-body text-sm font-medium px-5 py-2.5 rounded-lg bg-f10-primary text-white hover:opacity-90 active:scale-[0.98] transition-all whitespace-nowrap"
            >
              Analyze Content →
            </button>
          </div>
      ...
      {status === "done" && reportHtml && (
        <div>
          <div className="flex items-center gap-4 mb-4">
            <span className="font-body text-sm text-gray-400">Content health</span>
      ...
      {status === "error" && (
        <div>
          <p className="font-body text-sm text-red-400 mb-3">{error ?? "Analysis failed."}</p>
          <button
