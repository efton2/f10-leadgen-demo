...
export default function ProposalGenerator({
  ...
  if (state === "idle") {
    return (
      <button
        onClick={generate}
        className="w-full bg-f10-primary text-white font-body text-sm font-semibold px-6 py-4 rounded-f10 hover:bg-[#C4870F] transition-colors text-center"
      >
        Generate Proposal
      </button>
    );
    ...
    <div className="bg-white rounded-f10 border border-f10-border p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              // Reset every downstream field, not just `state` — otherwise a
              // freshly-regenerated (never-sent) proposal could still render
              // the stale "Proposal sent to {email}" banner from a previous
              // send, hiding the send form entirely.
              setState("idle");
              setSendState("idle");
              setEmail("");
              setCopied(false);
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="bg-f10-primary text-white font-body text-sm font-semibold px-5 py-2.5 rounded-f10 hover:bg-[#C4870F] transition-colors disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
              >
            ...
            <p className="font-body text-xs text-gray-400 text-center mt-4">
              Generated by Function 10 Media AI &nbsp;&bull;&nbsp; {`$${PRICING.standard.setup}`} setup &nbsp;&bull;&nbsp; {`$${PRICING.standard.monthly}/month`}
            </p>
          </div>
        )}
      </div>
