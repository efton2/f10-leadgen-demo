// Competitive analysis enrichment. Extracted from app/api/competitive-analysis/route.ts
// so the manual SSE route and the prospecting batch runner share one implementation.
...
export async function runApifyActor(actorId: string, input: object, meter?: UsageMeter): Promise<unknown[]> {
...
export function nameMatchScore(input: string, result: string): number {
...
export async function scrapeGoogleMaps(name: string, market: string, meter?: UsageMeter) {
...
// Renamed from scrapeWebsite to avoid collision with the audit module's scraper.
export async function crawlCompetitorSite(url: string | null, meter?: UsageMeter): Promise<string> {
  if (!url) return "";
  try {
...
export async function synthesize(
  ...
  brandName: string, competitorsRaw: object[], meter?: UsageMeter
) {
...
// Renamed from buildHtml for clarity at import sites.
export function buildCompetitiveHtml(data: Record<string, unknown>, brandKey: string): string {
